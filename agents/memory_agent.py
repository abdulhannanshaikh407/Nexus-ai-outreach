...
class MemoryAgent:
    ...
    def __init__(self, llm_model: str = "gpt-3.5-turbo", temperature: float = 0.1,
        ...
        self.faiss_available = FAISS_AVAILABLE  # Use global import status
        ...
        logger.info(f"Memory Agent initialized with {'FAISS' if self.faiss_available else 'JSON'} storage")
    ...
    def store_interaction(self, interaction_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.faiss_available and self.embedder:
    ...
    def retrieve_similar_interactions(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            if self.faiss_available and self.embedder and len(self.metadata) > 0:
    ...
    def get_performance_stats(self, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            if not self.metadata:
                return {"total_interactions": 0, "message": "No interactions stored yet"}
            filtered_data = self.metadata
            if filters:
                filtered_data = self._apply_filters(self.metadata, filters)
            if not filtered_data:
                return {"total_interactions": 0, "filtered_count": 0, "message": "No interactions match the filters"}
            ...
            logger.error(f"Error getting performance stats: {str(e)}")
            return {"error": str(e), "total_interactions": len(self.metadata) if self.metadata else 0}
        ...
        for key, value in filters.items():
            if key == "date_range" and isinstance(value, dict):
                ...
                if start_date or end_date:
                    filtered = [item for item in filtered if self._is_in_date_range(item.get("timestamp"), start_date, end_date)]
            elif key == "status":
    ...
    def _analyze_subject_performance(self, interactions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        personalized_count = sum(1 for s in subjects if any(indicator in s.lower() for indicator in ["{", "}", "first_name", "company", "name"]))
...
- Embedding Model: {'all-MiniLM-L6-v2' if self.faiss_available and self.embedder else 'None (text matching)'}
